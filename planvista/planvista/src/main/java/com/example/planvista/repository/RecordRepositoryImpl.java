 ...
 * V4のrecordsテーブル構造(task_id, task_name, event_id)に対応
 */
@Repository
public class RecordRepositoryImpl implements RecordRepository {
    ...
    public RecordEntity create(RecordEntity record) {
        String sql = "INSERT INTO records (user_id, event_id, task_id, task_name, start_time, end_time, memo, created_at, updated_at) " +
        ...
        jdbcTemplate.update(connection -> {
            ...
            if (record.getEventId() != null) {
                ps.setLong(2, record.getEventId());
            } else {
    ...
    public List<RecordEntity> findByUserId(Long userId) {
        String sql = "SELECT id, user_id, event_id, task_id, task_name, start_time, end_time, memo, created_at, updated_at " +
    ...
    public List<RecordEntity> findByUserIdAndTaskNameAndDateRange(Long userId, String taskName, LocalDateTime startDate, LocalDateTime endDate) {
        String sql = "SELECT id, user_id, event_id, task_id, task_name, start_time, end_time, memo, created_at, updated_at " +
                     "FROM records WHERE user_id = ? AND task_name = ? AND start_time >= ? AND start_time < ? ORDER BY start_time ASC";
        return jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(RecordEntity.class), 
    ...
    public List<RecordEntity> findByTaskId(Long taskId) {
        String sql = "SELECT id, user_id, event_id, task_id, task_name, start_time, end_time, memo, created_at, updated_at " +
                     "FROM records WHERE task_id = ? ORDER BY start_time DESC";
    ...
    public List<RecordEntity> findByUserIdAndTaskId(Long userId, Long taskId) {
        String sql = "SELECT id, user_id, event_id, task_id, task_name, start_time, end_time, memo, created_at, updated_at " +
                     "FROM records WHERE user_id = ? AND task_id = ? ORDER BY start_time DESC";
    ...
    public RecordEntity findByScheduleId(Long scheduleId) {
        // event_idで検索（V4ではevent_idがschedule_idを意味する）
        String sql = "SELECT id, user_id, event_id, task_id, task_name, start_time, end_time, memo, created_at, updated_at " +
                     "FROM records WHERE event_id = ? LIMIT 1";
    ...
    public void update(RecordEntity record) {
        String sql = "UPDATE records SET event_id = ?, task_id = ?, task_name = ?, start_time = ?, end_time = ?, memo = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?";
